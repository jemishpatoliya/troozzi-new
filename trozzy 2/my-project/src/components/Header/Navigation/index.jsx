...
import { IoMenuOutline, IoRocketSharp } from "react-icons/io5";
import { LiaAngleDownSolid } from "react-icons/lia";
import { Link } from 'react-router-dom';
import Categorypepal from './Categorypepal';
import './style.css';

const Navigation = () => {
  const [isopenCatpanel, setopenCategorypepal] = useState(false);

  const toggleCategorypepal = () => {
    setopenCategorypepal((prev) => !prev);
  };
  ...
  return (
    <>
      <nav className='py-3'>
        <div className='container flex items-center justify-between'>
          ...
          <div className='col_1 flex-shrink-0'>
            <Button
              className='!text-text-900 dark:!text-text-100 gap-2 font-semibold w-full bg-surface-50 dark:bg-surface-800 hover:bg-surface-100 dark:hover:bg-surface-700 rounded-xl transition-colors duration-200 border border-border-200 dark:border-border-700 shadow-sm'
              onClick={toggleCategorypepal}
            >
              <IoMenuOutline className='text-text-700 dark:text-text-300 text-[24px]' />
              <span className="text-sm">Shop by category</span>
              <LiaAngleDownSolid className='text-text-700 dark:text-text-300 text-[18px] ml-auto font-bold' />
            </Button>
          </div>
      ...
      <Categorypepal
        ...
        setopenCategorypepal={setopenCategorypepal}
      />
    </>
  );
};
