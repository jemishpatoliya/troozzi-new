...
const ProductCard = ({ product, view = "grid" }) => {
  ...
  const normalized = useMemo(() => {
    ...
    const defaultVariant = hasColorVariants ? product.colorVariants[0] : null;
    const currentVariant = selectedColorVariant || defaultVariant;

    // Get image from variant or fallback to product image
    const image = currentVariant?.images?.[0] || product.image || product.img || product.galleryImages?.[0] || "";
    ...
    const stock = Number(currentVariant?.stock ?? product.stock ?? 0);
    ...
    return {
      ...
      price,
      image,
      galleryImages: currentVariant?.images || Array.isArray(product.galleryImages) ? product.galleryImages : [],
      isSaleActive,
      saleDiscount,
      displayPrice,
      rating,
      ...
      >
        <div className="transition-transform duration-300">
          ...
          <div className="p-3 space-y-2">
            ...
            {/* Brand */}
            {normalized.brand && (
              <div className="text-xs text-gray-500">{normalized.brand}</div>
            )}

            {/* Color Picker */}
            {normalized.hasColorVariants && (
              <div className="py-2">
                <ColorPicker
                  colors={normalized.colorVariants}
                  selectedColor={normalized.currentVariant?.color}
                  onColorSelect={(color) => {
                    const variant = normalized.colorVariants.find(v => v.color === color.color);
