...
router.get("/my", authenticateToken, async (req, res) => {
    ...
    const missingOrderIds = paymentOrderIds.filter((id) => !directDocs.some((d) => String(d._id) === id));
    ...
    if (missingOrderIds.length) {
        await OrderModel.updateMany({ _id: { $in: missingOrderIds }, user: { $exists: false } }, { $set: { user: userObjectId } });
    }
...
router.put("/:id/status", async (req, res) => {
    ...
    const updated = await OrderModel.findByIdAndUpdate(req.params.id, { status: parsed.data.status }, { new: true }).lean();
    if (!updated)
