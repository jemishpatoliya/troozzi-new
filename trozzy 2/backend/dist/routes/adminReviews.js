...
router.delete("/:reviewId", adminAuth, async (req, res) => {
    try {
        ...
        const result = await ProductModel.updateOne({ "reviews._id": reviewId }, { $pull: { reviews: { _id: reviewId } } });
...
router.put("/:reviewId/status", adminAuth, async (req, res) => {
    try {
        ...
        const result = await ProductModel.updateOne({ "reviews._id": reviewId }, { $set: { "reviews.$.status": status } });
        if (result.modifiedCount === 0) {
            return res.status(404).json({ message: "Review not found" });
        }
...
router.put("/bulk-status", adminAuth, async (req, res) => {
    try {
        ...
        const result = await ProductModel.updateMany({ "reviews._id": { $in: objectIds } }, { $set: { "reviews.$.status": status } });
        res.json({
