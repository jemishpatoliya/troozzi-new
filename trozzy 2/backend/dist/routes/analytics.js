...
router.get("/overview", authenticateAdmin, async (req, res) => {
    try {
        ...
        const revenueByDay = new Map(revenueAgg.map((r) => [r._id, Number(r.total ?? 0)]));
        const ordersByDay = new Map(ordersAgg.map((r) => [r._id, Number(r.count ?? 0)]));
...
router.get("/advanced", authenticateAdmin, async (req, res) => {
    try {
...
router.get("/realtime", authenticateAdmin, async (_req, res) => {
    try {
        const now = new Date();
...
router.get("/bi", authenticateAdmin, async (req, res) => {
    try {
        const dateFilter = String(req.query.dateFilter ?? "30d");
        const categoryFilter = String(req.query.categoryFilter ?? "all");
        const products = await ProductModel.find({}).select({ name: 1, price: 1, stock: 1, category: 1 }).lean();
