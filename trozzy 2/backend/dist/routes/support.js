...
router.get('/all', async (req, res) => {
    try {
        const { status, userId, category, priority, dateFrom, dateTo, page = 1, limit = 20, sortBy = 'createdAt', sortOrder = 'desc' } = req.query;
        // Build filter object
        const filter = {};
        if (status)
...
router.post('/ticket', async (req, res) => {
    try {
        const { userId, userEmail, userName, category, orderId, subject, message, priority = 'medium' } = req.body;
...
router.put('/:ticketId/reply', async (req, res) => {
    try {
        ...
        const updatedTicket = await SupportTicketModel.findOneAndUpdate({ ticketId }, {
            $set: {
                adminReply: message,
...
router.put('/:ticketId/status', async (req, res) => {
    try {
        ...
        const updatedTicket = await SupportTicketModel.findOneAndUpdate({ ticketId }, { $set: updateData }, { new: true });
        res.json({
            success: true,
            data: updatedTicket,
