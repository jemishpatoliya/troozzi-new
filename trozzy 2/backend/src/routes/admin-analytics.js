...
router.get('/overview', authenticateAdmin, async (req, res) => {
    try {
        ...
        const pageViews = series.map((date) => {
            ...
            return { date, views: Math.round(v * 1.35) };
        });
...
router.get('/realtime', authenticateAdmin, async (_req, res) => {
    try {
        ...
        const [ordersLastMinute, revenueLastMinuteAgg, customers] = await Promise.all([
            ordersCol.countDocuments({ createdAt: { $gte: from, $lte: now } }),
            ordersCol
                .aggregate([
                    {
                        $match: {
                ...
                .toArray(),
            customersCol.countDocuments({ role: 'user' }),
        ]);
...
router.get('/bi', authenticateAdmin, async (req, res) => {
    try {
        ...
        const products = await productsCol
            .find({}, { projection: { name: 1, price: 1, stock: 1, category: 1 } })
            .limit(200)
            .toArray();
