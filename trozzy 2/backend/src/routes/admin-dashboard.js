...
router.get('/analytics/realtime', authenticateAdmin, async (_req, res) => {
  try {
    ...
    notices.push('Active Users currently reflects admin logins only. User session tracking is not available in this project yet.');
...
router.get('/analytics/bi', authenticateAdmin, async (req, res) => {
  try {
    ...
    notices.push('Profit vs Loss: No sufficient data');
...
router.get('/dashboard', authenticateAdmin, async (req, res) => {
  try {
    ...
    // Calculate revenue from products (since no orders exist)
    const revenueData = await ProductModel.aggregate([
      { $match: { status: 'active' } },
      { $group: { _id: null, totalValue: { $sum: { $multiply: ['$price', '$stock'] } } } }
    ]);
    const totalRevenue = revenueData.length > 0 ? revenueData[0].totalValue : 0;

    const currentRevenuePeriodAgg = await ProductModel.aggregate([
      { $match: { status: 'active' } },
      {
        $addFields: {
          _createdAtDate: {
    ...
    const currentRevenuePeriod = currentRevenuePeriodAgg.length > 0 ? currentRevenuePeriodAgg[0].totalValue : 0;

    const previousRevenuePeriodAgg = await ProductModel.aggregate([
      { $match: { status: 'active' } },
      {
        $addFields: {
          _createdAtDate: {
      ...
      { $match: { _createdAtDate: { $gte: previousStartDate, $lt: startDate } } },
      { $group: { _id: null, totalValue: { $sum: { $multiply: ['$price', '$stock'] } } } },
    ]);
    const previousRevenuePeriod = previousRevenuePeriodAgg.length > 0 ? previousRevenuePeriodAgg[0].totalValue : 0;
